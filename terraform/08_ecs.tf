...
resource "aws_ecs_task_definition" "client-app" {
  ...
  memory                   = "512"
  cpu                      = "256"
  container_definitions    = templatefile("templates/client_app.json.tpl", {docker_image_url_client = var.docker_image_url_client, region = var.region}) 
}
...
resource "aws_ecs_task_definition" "users-app" {
  ...
  memory                   = "512"
  cpu                      = "256"
  container_definitions    = templatefile("templates/users_app.json.tpl", {docker_image_url_users = var.docker_image_url_users, region = var.region, secret_key = var.secret_key, database_url = "postgres://webapp:${var.rds_password}@${aws_db_instance.production.endpoint}/api_prod"}) 
